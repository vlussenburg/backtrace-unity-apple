...
+ (NSString*) getDefaultReportPath {
    return [ [Utils getCacheDir] stringByAppendingPathComponent:@"Backtrace.plist"];
}

// returns path to oom status file path
+ (NSString*) getDefaultOomStatusPath {
    return [ [Utils getCacheDir] stringByAppendingPathComponent:@"BacktraceOOMState.json"];
}

+ (BOOL) prepareCrashDirectory {
     NSString* backtraceDir = [Utils getCacheDir];
    BOOL isDir = NO;
      NSError *error;
